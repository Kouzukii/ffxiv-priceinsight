...
public class UniversalisClient : IDisposable {
    ...
    private static readonly Dictionary<uint, uint> WorldToDc = Service.DataManager.GetExcelSheet<World>()!.Where(w => w.IsPublic)
    ...
    private MarketBoardData ParseMarketBoardData(ItemData item, uint worldId) {
        ...
        var marketBoardData = new MarketBoardData {
            ...
            MinimumPriceNQ = item.listings?.FirstOrDefault(l => !l.hq && WorldToDc[l.worldID] == dc),
            MinimumPriceHQ = item.listings?.FirstOrDefault(l => l.hq && WorldToDc[l.worldID] == dc),
            ...
            MostRecentPurchaseNQ = item.recentHistory?.FirstOrDefault(l => !l.hq && WorldToDc[l.worldID] == dc),
            MostRecentPurchaseHQ = item.recentHistory?.FirstOrDefault(l => l.hq && WorldToDc[l.worldID] == dc),
    ...
    public class ListingData {
        ...
        public static implicit operator Listing?(ListingData? data) {
            if (data == null)
                return null;
            return new Listing {
                Price = data.pricePerUnit, Time = data.lastReviewTime, World = data.worldName, Datacenter = UniversalisClient.WorldToDcName[data.worldID]
            };
        }
    }
    ...
    public class RecentData {
        ...
        public static implicit operator Listing?(RecentData? data) {
            if (data == null)
                return null;
            return new Listing {
                Price = data.pricePerUnit, Time = data.timestamp, World = data.worldName, Datacenter = UniversalisClient.WorldToDcName[data.worldID]
            };
        }
    }
}
